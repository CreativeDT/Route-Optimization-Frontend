...
//   };
  ...
  ? data
  : data.filter((v) => v.fuel_type?.trim().toLowerCase() === filter.toLowerCase());

  const paginatedData =
    tabIndex === 0
      ? data.slice(userPage * userRowsPerPage, userPage * userRowsPerPage + userRowsPerPage)
      : filteredVehicles.slice(vehiclePage * vehicleRowsPerPage, vehiclePage * vehicleRowsPerPage + vehicleRowsPerPage);
...
const handleSaveVehicle = () => {
    ...
    if (data.some(vehicle => vehicle.LicenseNo === newVehicle.LicenseNo && (!editingVehicle || vehicle.VehicleID !== editingVehicle.VehicleID))) {
          ...
          {tabIndex === 0 ? (
              <Box className="filter-container">
                <Button
                  ...
                  color="primary"
                  startIcon={<Add />}
                  onClick={() => handleOpenDialog()} // Replace with your handleOpenDialog function
                >
                  Create User
                </Button>
              ...
              >
                    ...
                    />
                    <Tab
                       label={`Managers (${data.filter((u) => u.role?.trim().toLowerCase() === "manager").length})`}
                      className="tab"
                      value="manager"
                      sx={{
                        backgroundColor:
                ...
                {paginatedData
                        .filter((item) => {
                            ...
                            if (tabIndex === 1 && filter !== "All") {
                                fuelTypeMatch = item.fuel_type?.trim().toLowerCase() === filter.toLowerCase();
                            }
                          ...
                          <>
                            ...
                            <TableCell>
                              <Switch
                                ...
                                disabled={
                                  loggedInUserRole &&
                                  loggedInUserRole.toLowerCase() === "admin"
                                }
                              />
                            </TableCell>
  ...
  <DialogContent sx={{ py: 3 }}>
    <Box component="form" noValidate sx={{ mt: 1 }}>
      ...
      <FormControl fullWidth size="small" sx={{ mb: 2.5 }}>
        <InputLabel sx={{ transform: 'translate(14px, -9px) scale(0.75)' }}>Vehicle Type</InputLabel>
        <Select
      ...
      {/* Fuel Type */}
      <FormControl fullWidth  size="small"  sx={{ mb: 2.5 }}>
        <InputLabel sx={{ transform: 'translate(14px, -9px) scale(0.75)' }}>Fuel Type</InputLabel>
        <Select
      ...
      <TextField
        ...
        sx={{ mb: 2.5 }}
        type="number"
        value={newVehicle.ExhaustCO2 ? (parseFloat(newVehicle.ExhaustCO2) * 1.60934).toFixed(2) : ''}
        onChange={(e) => {
          const value = e.target.value;
          if (/^\d*\.?\d*$/.test(value)) {
            setNewVehicle({ 
              ...newVehicle, 
              ExhaustCO2: value ? (parseFloat(value) / 1.60934).toFixed(2) : '' 
            });
          }
        }}
        InputProps={{
      ...
      <TextField
        ...
        sx={{ mb: 2.5 }}
        type="number"
        value={newVehicle.Mileage ? (2.3521458 / parseFloat(newVehicle.Mileage)).toFixed(1) : ''}
        onChange={(e) => {
          const value = e.target.value;
          if (/^\d*\.?\d*$/.test(value)) {
            setNewVehicle({ 
              ...newVehicle, 
              Mileage: value ? (2.3521458 / parseFloat(value)).toFixed(4) : '' 
            });
          }
        }}
        InputProps={{
      ...
      <TextField
        ...
        sx={{ mb: 2.5 }}
        type="number"
        value={newVehicle.VehicleCapacity ? (parseFloat(newVehicle.VehicleCapacity) * 2.20462).toFixed(0) : ''}
        onChange={(e) => {
          const value = e.target.value;
          if (/^\d*\.?\d*$/.test(value)) {
            const numericValue = value === "" ? "" : parseFloat(value) / 2.20462;
            setNewVehicle({
              ...newVehicle,
      ...
      <FormControl fullWidth   size="small" sx={{ mb: 1 }}>
        <InputLabel sx={{ transform: 'translate(14px, -9px) scale(0.75)' }}>Vehicle Status</InputLabel>
        <Select
